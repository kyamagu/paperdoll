...
for t = 1:iter,
  ...
  for i = 1:length(neg)
    ...
    bbox = [];
    if isfield(neg, 'point') && ~isempty(neg(i).point)
      bbox = [neg(i).x1 neg(i).y1 neg(i).x2 neg(i).y2]';
    end
    [box, model] = detect(neg(i), model, -1, bbox(:)', overlap, i, -1);
    % END HACK %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:numpos
  ...
  feat = features(warped(i).image, model.sbin);
  if isfield(warped, 'context')
    feat = cat(3, feat, compute_context_features(warped(i).context, model));
  end
  % END HACK %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:numpos
  ...
  image = imread_or_decode(pos(i).image, 'jpg');
  [sample.image, cropped_bbox] = croppos(image, bbox);
  if isfield(pos, 'context')
    sample.context = croppos(imread_or_decode(pos(i).context, 'png'), bbox);
  end
  box = detect(sample, model, 0, cropped_bbox, overlap, i, 1);
  % END HACK %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if ~isempty(box),
