...
if nargin > 3 && ~isempty(bbox) && label >= 0
% END %%%%%
  latent = true;
  thresh = -INF;
else
    ...
    for i = 1:length(X)
      ...
      if write && ~latent
        ...
        if ~isempty(bbox)
          ...
          rate = boxoverlap(bbox, box);
          if nnz(rate>overlap)/numel(rate) > overlap/2, continue; end
        end
        last_rscore = rscore(y,x); % keep this for assertion
        qp_write(ex);
        qp.ub  = qp.ub + qp.Cneg*max(1+last_rscore,0);
        %qp_write(ex);
