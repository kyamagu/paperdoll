function pyra = featpyramid(sample, model, scale)
...
% BEGIN %%%%%%%%%%%%%%%%%%
assert(isstruct(sample) && isfield(sample, 'image'));
...
for i = 1:interval
  ...
  if ~isempty(context)
    scaled_context = imresize(context, ...
                              [size(scaled,1), size(scaled,2)], ...
                              'nearest');
    pyra.feat{i} = cat(3, ...
                       pyra.feat{i}, ...
                       compute_context_features(scaled_context, model));
  end
  % END %%%%%%%%%%%%%%%%%%%%
